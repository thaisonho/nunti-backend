...
```bash
INIT=$(node "/home/json/hcmus/applied_crypto/nunti-backend/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "/home/json/hcmus/applied_crypto/nunti-backend/.claude/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
