...
```bash
PHASE_INFO=$(node "./.augment/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "./.augment/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "./.augment/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
