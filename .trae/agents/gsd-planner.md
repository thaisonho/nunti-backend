...
```bash
node "./.trae/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
@./.trae/get-shit-done/references/thinking-models-planning.md
...
```bash
VALID=$(node "./.trae/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "./.trae/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "./.trae/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
